# YDollar (experimental)

YDollar is a federated, over-collateralised US-dollar stablecoin **overlay** on Ycash. It is a
Tier-0 feature: no consensus change, no policy change, no new opcodes and no network upgrade. A
YDollar balance is an ordinary transparent output whose dollar value is declared in the
...
spending a well-known anchor UTXO. Every node that runs with `-ydollar` computes the same YDollar
state from the same chain in a self-contained, rebuildable index under `<datadir>/ydollar/`.
...
develops this fork (`docs/plans/ydollar-v1-development-plan.md`); this file is the user-facing
...
| 1 — pure protocol library (`src/ydollar/{params,amount,payload,script,address}`) | not started |
| 2 — state machine, index, node RPCs | not started |
| 3 — wallet RPCs (mint, send, redeem, co-sign) | not started |
| 4 — federation coordinator (`contrib/ydollar/`) | not started |
...
`-ydollar` refuses to start with `-prune` (the index rebuilds from blocks on disk). Other
options: `-reindex-ydollar` (wipe and rebuild the index), `-ydollarfee=<zat>` (flat fee, minimum
1000), `-ydollarmintlag=<blocks>` (default 2), `-debug=ydollar`. Regtest additionally takes
`-ydollarstartheight`, `-ydollargenesisanchor`, `-ydollargenesisroster` (all three together) and
`-ydollarsupplycap`.

...
> **Phase 0 (v2):** this is the federation prototype's contract, being replaced by phase; the v2
> contract (rpcversion 2) arrives with Phase 3 and is checked mechanically against
> `doc/yellowback-rpc-contract.json`. The federation commands (`yed_getroster`,
> `yed_createpricetx`, `yed_submitredeem`, `yed_abortredeem`, `yed_cosignredeem`) and the
> operator `/cosign` endpoint no longer exist; `yed_redeem` still builds and owner-signs the v1
> redemption but nothing can complete it (the vault script needs the retired federation's
> signatures), so redemption is out of service until Phase 6.
...
| `yed_listunspent` | — | `[{txid,vout,cents,valueZat,address,height,confirmations,spentUnconfirmed,locked}]` |
...
| `yed_redeem` | `vaultTxid [to]` | `hex` (owner-signed, not broadcastable in Phase 0-5), `vault`, `requiredBurnCents`, `burnCents`, `changeCents`, `collateralTo`, `shielded`, `expiryHeight`. Refusals: `locked until height`, `insufficient YED`, `RED-6` (no price) |
| `yed_listpositions` | `[status]` | `[{vaultTxid,status,mintedCents,collateralZat,lockHeight,unlockHeight,tier,mintHeight,rosterIndex,ownerKeyId,requiredBurnCents,canRedeem, voidReason \| closeHeight,closingTxid,burnedCents}]` |

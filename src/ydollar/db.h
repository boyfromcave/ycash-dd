...
#ifndef YCASH_YDOLLAR_DB_H
#define YCASH_YDOLLAR_DB_H
...
/**
 * LevelDB-backed StateView under <datadir>/ydollar/ (plan D8), on the same
...
class YDollarDB : public StateView
{
public:
    YDollarDB(const fs::path& path, size_t nCacheSize, bool fMemory = false, bool fWipe = false);
    ...
    bool Sync();
    /** Delete every key (used by -reindex-ydollar and by the start-empty fallbacks). */
    bool Wipe();
